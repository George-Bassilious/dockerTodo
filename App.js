...
export default function App() {
  ...
  return (
    <Box position="absolute" top="15%" left="15%">
      ...
      <input id="newTodo" placeholder="new to-do" onChange={() => setnewTodo(event.target.value)}></input>
      ...
      <div>{
        todos.map(todo => 
          <Box bgcolor="#D6EBEF" p={1} margin="10px" textAlign="center" fontFamily="sans-serif">
            <p>{todo + " "}<button onClick={() => deleteTodo(todo)}> X </button></p>
          </Box>
        )
      }</div>
